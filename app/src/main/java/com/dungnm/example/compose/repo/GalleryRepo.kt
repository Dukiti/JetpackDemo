...
import com.dungnm.example.compose.model.AlbumGallery
    ...
    fun getListGalleryPhoto(
        ...
        cursorPhoto?.use { cursor ->
            if (cursor.moveToFirst()) {
                do {
                    val path =
                        cursor.getString(cursor.getColumnIndex(MediaStore.Images.ImageColumns.DATA).coerceAtLeast(0))
                    val id =
                        cursor.getString(cursor.getColumnIndex(MediaStore.Images.ImageColumns._ID).coerceAtLeast(0))
